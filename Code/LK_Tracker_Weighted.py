...
def lucasKanadeTracker(I, Template, rectangle_coords, p):
    ...
    while True:
        ...
        # Calculate weight from error.
        weight = getRobustError(error)
        ...
        Hessian_mat = np.matmul(SDI.T, SDI * weight)

        # SDI Parameter Update------------------------------------------STEP 7
        SDI_param_update = np.matmul(SDI.T, error * weight)
...
PATHS = ['Car4/img/', 'Bolt2/img/','DragonBaby/img/']
Dataset_name = ['Car4', 'Bolt2', 'DragonBaby']

# Setting output folder
folders = ['OUTPUT_CAR_WEIGHTED', 'OUTPUT_BOLT_WEIGHTED', 'OUTPUT_BABY_WEIGHTED']
...
for ind, image_PATH in enumerate(PATHS):
    ...
    if folders[ind] == 'OUTPUT_BABY_WEIGHTED':
        image_size = (640, 360)

    if folders[ind] == 'OUTPUT_CAR_WEIGHTED':
        image_size = (360, 240)

    if folders[ind] == 'OUTPUT_BOLT_WEIGHTED':
        image_size = (480, 270)

    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(folders[ind] + '_video1.avi', fourcc, 10, image_size)
    ...
    for im in range(0, dataset_size):
        ...
        if Dataset_name[ind] == 'Car4':
            ...
            if it > 310:
                center_x = np.abs(newStart[0] + newEnd[0]) / 2 + 0.85 * rect_width
                center_y = np.abs(newStart[1] + newEnd[1]) / 2
        ...
        cv2.imwrite(folders[ind] + '/' + str(it) + '.jpg', img)
